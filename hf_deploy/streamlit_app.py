...
Uses matplotlib for plots - HF Spaces compatible
"""
...
def create_matplotlib_plot(f1, f2, r, scale, theme):
...
with st.sidebar:
    ...
    f1_input_col1, f1_input_col2 = st.columns([3, 1])
    with f1_input_col1:
        f1_angle_slider = st.slider("Angle (°):", 0, 360, 30, key="f1_angle_slider")
    with f1_input_col2:
        f1_angle = st.number_input("°", min_value=0, max_value=360, value=f1_angle_slider, step=1, key="f1_angle", label_visibility="collapsed")
    
    # Sync slider with number input
    if f1_angle != f1_angle_slider:
        f1_angle_slider = f1_angle
    ...
    f2_input_col1, f2_input_col2 = st.columns([3, 1])
    with f2_input_col1:
        f2_angle_slider = st.slider("Angle (°):", 0, 360, 120, key="f2_angle_slider")
    with f2_input_col2:
        f2_angle = st.number_input("°", min_value=0, max_value=360, value=f2_angle_slider, step=1, key="f2_angle", label_visibility="collapsed")
    
    # Sync slider with number input
    if f2_angle != f2_angle_slider:
        f2_angle_slider = f2_angle
    st.divider()
...
if calculate_btn or 'last_result' in st.session_state:
    try:
        ...
        with col1:
            ...
            fig = create_matplotlib_plot(f1, f2, r, scale, st.session_state.theme)
            st.pyplot(fig)
    ...
    with st.expander("Quick Start Guide"):
        ...
        **How to use:**
        1. Enter force magnitudes and angles in sidebar
        2. Click Calculate to visualize
        3. Enable "Show Steps" for detailed solution
...
st.divider()
st.caption("Built with Streamlit + Matplotlib • Vector Calculator v2.0")
